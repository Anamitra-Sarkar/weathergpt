...
def _card(bundle: dict, repo_id: str) -> str:
    lines = [
        ...
        "```", "",
        "Every model attribute is `None` if its admission gate failed — always",
        "check for `None` before calling it and fall back to a deterministic",
        "path. This bundle's gate results: " +
        ", ".join(f"`{name}` {'PASS' if b['gate_ok'] else 'REFUSED'}"
                 for name, b in bundle.items()) + ".", "",
        "## Usage — one real call per model", "",
        "These are the actual method signatures, not illustrative pseudocode —",
        "copied from `modal_jobs/verify_package.py` and",
        "`modal_jobs/realworld_check.py`, which exercise every one of them",
        "against live data before each release.", "",
        "```python", "# M1 -- map a raw provider field name to a canonical variable",
        ...
        "```", "",
        "Full integration guidance for wiring this into a FastAPI service (which",
        "site in the codebase to touch, what NOT to change, live-tested caveats",
